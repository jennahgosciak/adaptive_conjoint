...
for (i in c(1:copies)) {
  ...
  fn <- if (r == "W") {white_first_name[name_order]} else {black_first_name[name_order]}
  ln <- if (r == "W") {white_last_name[name_order]} else {black_last_name[name_order]}
  
  univ_order <- sample(c(1,2), replace=F)
  univ_rank <- sample(university_rank, 1)
  univ <- if (univ_rank == "High") {university_highrank[univ_order]} else {university_lowrank[univ_order]}
  ...
  for (j in c(1:2)) {
    doc <- read_docx(str_glue("resume_format{format[j]}.docx")) %>% 
      body_replace_all_text(old_value = "FirstName",
      new_value = fn[j], fixed = TRUE) %>% 
      body_replace_all_text(old_value = "firstname",
      new_value = str_to_lower(fn[j]), fixed = TRUE) %>%
      body_replace_all_text(old_value = "LastName",
      new_value = ln[j], fixed = TRUE) %>% 
      body_replace_all_text(old_value = "lastname",
      new_value = str_to_lower(ln[j]), fixed = TRUE) %>% 
      body_replace_all_text(old_value = "University",
      new_value = univ[j], fixed = TRUE) %>% 
      body_replace_all_text(old_value = "TreatmentDescription",
                            new_value = t_desc[j], fixed = TRUE) %>%
      body_replace_all_text(old_value = "TreatmentHeader",
                            new_value = t_head[j], fixed = TRUE) %>%
      body_replace_all_text(old_value = "Treatment",
                            new_value = t[j], fixed = TRUE) %>% 
      body_replace_all_text(old_value = "YEAR",
