...
levels_per_category <-lst("university" = c("lowrank", "highrank"), 
                          "race" = c("black", "white"))
...
```{r, include=F}
first_names <- lst("white" = c("Jessica", "Ashley"),
                   "black" = c("Ebony", "Aisha"))

last_names <- lst("white" = c("Olsen", "Snyder"),
                   "black" = c("Washington", "Jefferson"))

university <- lst("high" = c("Massachusetts Institute of Technology", "University of Pennsylvania"),
                  "low" = c("University of Denver", "University of Arizona"))
```

```{r}
treatment <- c("Parent Teacher Association Volunteer", 
                 "Marketing Coordinator")
treatment_header <- c("| Creekside Elementary | Denver, CO 80014 | 2022 – present",
                        "| Goods Delivered | Denver, CO | 2021 to Present")
treatment_description <- c("Member of the Executive Board. Served as Secretary.",
...
for (c in 1:nrow(levels_grid)) {
    ...
    for (j in 1:2) {
      doc <- doc %>% 
          body_replace_all_text(old_value = str_glue("FirstName{j}"),
          new_value = rand_data$first_names[[j]], fixed = TRUE) %>% 
          body_replace_all_text(old_value = str_glue("firstname{j}"),
          new_value = str_to_lower(rand_data$first_names[j]), fixed = TRUE) %>%
          body_replace_all_text(old_value = str_glue("LastName{j}"),
          new_value = rand_data$last_names[j], fixed = TRUE) %>% 
          body_replace_all_text(old_value = str_glue("lastname{j}"),
          new_value = str_to_lower(rand_data$last_names[2]), fixed = TRUE) %>% 
          body_replace_all_text(old_value = str_glue("University{j}"),
          new_value = rand_data$university[j], fixed = TRUE) %>% 
          body_replace_all_text(old_value = str_glue("TreatmentDescription{j}"),
                                new_value = t_desc[j], fixed = TRUE) %>%
          body_replace_all_text(old_value = str_glue("TreatmentHeader{j}"),
                                new_value = t_head[j], fixed = TRUE) %>%
          body_replace_all_text(old_value = str_glue("Treatment{j}"),
                                new_value = t[j], fixed = TRUE) %>% 
          body_replace_all_text(old_value = str_glue("YEAR{j}"),
