update_outcomes <- function(df, num_contexts, num_outcome, outcome_value) {
  ...
  obs_outcome <- table(df[df[["outcome"]] == outcome_value, "context"])[context_cols] %>%
    unname()
...
update_ts <- function(df, varname, num_sim, num_contexts, cdf, type) {
  # with the data provided
  # calculated the observed outcomes = 1, and outcomes = 0

  # uncomment this for the vectorized approach
  num_outcome1 <- update_outcomes(df, num_contexts, integer(num_contexts), 1)
  num_outcome0 <- update_outcomes(df, num_contexts, integer(num_contexts), 0)
