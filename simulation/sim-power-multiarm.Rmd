...
```{r, include=FALSE}
set.seed(1234)
est_avgn <- suppressMessages(future_map_dfr(c(9, 12, 16, 18, 20), 
                    ~produce_avgn(200, seq(0.3, 0.7, length.out = .), 
                         num_sim = 100)))
...
adaptive_phase <- function(warmup_df, n_total, true_p, n_sim=1000) {
...
mean_estimates_warmupfixed_3 <- future_map_dfr(1:n_sim, ~adaptive_warmupfixed2(50000, seq(0.3, 0.7, length.out = 9)), 
  ...
  mutate(num_arms = 12) %>% 
  filter(context == num_arms)) %>% 
  bind_rows(future_map_dfr(1:n_sim, ~adaptive_warmupfixed2(50000, seq(0.3, 0.7, length.out = 16)), .options=furrr_options(seed=TRUE)) %>% 
  mutate(num_arms = 16) %>% 
  filter(context == num_arms)) %>% 
  bind_rows(future_map_dfr(1:n_sim, ~adaptive_warmupfixed2(50000, seq(0.3, 0.7, length.out = 20)), .options=furrr_options(seed=TRUE)) %>% 
  mutate(num_arms = 20) %>% 
  filter(context == num_arms))
toc()

mean_estimates_warmupfixed_3
```
