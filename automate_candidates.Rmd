...
ln_list <- ln_list %>% 
  rename_at(c("PERCENT NON-HISPANIC OR LATINO WHITE ALONE",
...
levels_per_category <-lst("race_gender" = c("black_female", "black_male",
...
career <- lst("all" = c("Restaurant owner", "Small business owner"))
...
for (c in 1:nrow(levels_grid)) {
    cat(str_c(" Race and gender: ",levels_grid[c, ][["race_gender"]], 
  ...
  for (i in 1:copies){
    ...
    rand_data$ages[i], " & ", rand_data$ages[j], "\\\\
