...
df <- df %>%
  mutate(
    ...
    batch_type = case_when(
      ...
      StartDate_clean <= ymd_hms("2023-12-30-11-03-00") ~ "Iterative Batch Phase: Min",
      TRUE ~ NA_character_
    )
  ) %>%
...
if (current_batch_num == 0) {
  temp_batch_type <- "Warmup"
} else {
  temp_batch_type <- current_batch_type
}

probabilities_current <- probabilities %>%
  filter(Batch == current_batch_num, `Batch Type` == temp_batch_type)

df %>%
  filter(batch_id == current_batch_num, batch_type == temp_batch_type) %>%
