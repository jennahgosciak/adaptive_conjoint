...
batch_num <- 5 # numeric value for updating the batch number
...
df <- df %>%
  mutate(
    batch_id = case_when(
      ...
      StartDate_clean <= ymd_hms("2023-12-30-10-16-00") ~ 5
    ),
    batch_type = case_when(
      batch_id == 0 ~ "Warmup",
      StartDate_clean <= ymd_hms("2023-12-30-10-16-00") ~ "Iterative Batch Phase: Min",
      TRUE ~ NA_character_
    )
  ) %>%
