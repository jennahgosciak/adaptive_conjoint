...
draw_posterior <- function(data) {
  posterior <- prior %>%
    left_join(data %>%
                group_by(x) %>%
                summarize(y1_yes = sum(y1),
                          y1_no = sum(1 - y1),
                          y2_yes = sum(y2),
                          y2_no = sum(1 - y2),
                          .groups = "drop"),
              by = "x") %>%
    mutate(alpha1_post = alpha1 + y1_yes,
...
posterior_parameters <- data %>%
  mutate(time = 1:n()) %>%
  group_by(x) %>%
  mutate(posterior_alpha1 = 1 + cumsum(y1),
...
posterior_ci <- foreach(i = 1:nrow(posterior_parameters), .combine = "rbind") %do% {
  ...
  sim <- sim1 - sim2
  return(data.frame(ci.min = quantile(sim, .025),
                    ci.max = quantile(sim, .975)))
}
rownames(posterior_ci) <- NULL
posterior_parameters %>%
  bind_cols(posterior_ci) %>%
  right_join(data.frame(x = rep(1:n_x, each = nrow(data)), 
