...
# of each batch.
# Filter for batch type = "Iterative Batch Phase: Min" or "Iterative Batch Phase: Max",
# as otherwise the batch IDs will get messed up 
aggregated <- data %>%
  filter(batch_type %in% c("Iterative Batch Phase: Min", "Warmup")) %>% 
...
aggregated %>%
  ...
  xlab("Batch")
ggsave("../02_output/_figures/all_data_estimates_political_candidates_min.pdf",
...
ggsave("../02_output/_figures/all_data_estimates_political_candidates_min.png",
  height = 3, width = 4, dpi = 300
)
```
```{r}
# Same as above using credible intervals
...
ggsave("../02_output/_figures/all_data_estimates_political_candidates_min_bayes.pdf",
...
```{r}
# validation phase, estimates
# no batches here, so process is simpler
