...
df <- df %>%
  ...
  filter(StartDate_clean >= ymd_hms("2023-12-14-17-40-00"))
...
df_clean %>%
  select(chose_younger, str_c("Q", 1:8), profile) %>%
...
df_clean %>%
  ...
  summarize(
    total_chose_younger = sum(chose_younger),
    percent_select_younger = mean(chose_younger)
  )
```
