...
df <- df %>%
  ...
  filter(StartDate_clean < ymd_hms("2025-01-01-00-00-00"))
...
```{r}
date_today <- str_replace_all(Sys.Date(), "-", "_")

df_clean %>%
  select(unique_id, context, context_label, batch_id, batch_type, chose_mother, race, age, female, hispanic, everything()) %>%
  select(-ResponseId, -`Prolific ID Q`) %>% 
  saveRDS(str_glue("../../02_output/job_applicants_data_clean_{date_today}.RDS"))

df_clean %>%
  select(unique_id, context, context_label, batch_id, batch_type, chose_mother, race, age, female, hispanic, everything()) %>%
  select(-ResponseId, -`Prolific ID Q`) %>% 
  write_csv(str_glue("../../02_output/job_applicants_data_clean_{date_today}.csv"))
```
