...
-   Consider the **Bernoulli Bandit:**
    ...
    -   At each time $t$, generate an estimate \$\\hat{theta_k\$}\$ of the mean reward for the $k$th action
...
-   Contextual decisionmaking (e.g., observing a weather report prior to select path)
...
## Thompson Sampling in R: A Multi-Armed Approach
...
#| code-summary: "expand for full code"
library(ggplot2)
ggplot(data = tibble("ads" = ads_selected), 
       aes(x = ads)) + 
...
df %>% 
  ...
  scale_x_discrete(breaks = c(1:10), labels=c(1:10)) +
  labs(x = "arm/action/ad", y="number selected",
       title = "Real clicks obesrved in the data") +
  theme_classic()
```
...
beta_dist <- map2(numbers_of_rewards_1, numbers_of_rewards_0,
    ~dbeta(x = seq(0.01,1, 0.01), 
      shape1 = .x,
      shape2 = .y))
